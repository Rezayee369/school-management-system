...
export default function AdminReportsPage() {
    ...
    const [isLoadingClassReport, setIsLoadingClassReport] = useState(true);
    const [isLoadingHighAbsenceReport, setIsLoadingHighAbsenceReport] = useState(true);

    // Effect for Class Attendance Report
    useEffect(() => {
        const generateClassReport = async () => {
            ...
            try {
                ...
                const finalReport = classes.map(c => {
                ...
                setClassReport(finalReport);
            } catch (error) {
                console.error("Failed to generate class report:", error);
                toast.error("Could not load class report data.");
            } finally {
    ...
    // Effect for High Absence Report
    useEffect(() => {
        const generateHighAbsenceReport = async () => {
            if (!db) return;
            setIsLoadingHighAbsenceReport(true);
            try {
                ...
                const finalReport = Object.entries(absencesByStudent)
                ...
                setHighAbsenceReport(finalReport);
            } catch (error) {
                console.error("Failed to generate high absence report:", error);
                toast.error("Could not load high absence report data.");
            } finally {
                setIsLoadingHighAbsenceReport(false);
            }
        };
        generateHighAbsenceReport();
    }, [db]);
